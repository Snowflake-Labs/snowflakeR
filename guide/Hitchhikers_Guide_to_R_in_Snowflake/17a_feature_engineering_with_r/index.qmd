...
This chapter covers **getting Feature Store training data into R** for modeling. It is organized in three layers:
...
Start with [Feature Store training data in R](#sec-feature-store-read) if you are new to exports; jump to [Choose a pipeline](#sec-e2e-pipelines) if features are already defined and you need a full workflow.
...
Warehouse logic appears as **SQL** or **dbplyr** (RSnowflake lazy tables) wherever both are practical — the same idea as the [RSnowflake: Query §dbplyr](../13_rsnowflake_query/index.qmd#sec-dbplyr) (`group="query-sql-dplyr"`). Examples here use **tabs** where your tab choice persists while you read this chapter. Use **dbplyr** when you already work in R with `tbl()`; use **SQL** when you share DDL with other teams or generate views from a feature manifest.
...
Use `save_as` for wide or large training sets: validate joins in Snowflake, then read with RSnowflake + **ADBC** ([ADBC setup](#sec-adbc-semistructured)):
...
2. **`save_as` table + RSnowflake + ADBC** — recommended for wide/scalar-heavy training frames at scale ([ADBC setup](#sec-adbc-semistructured)). Dataset Parquet still stores VARIANT as JSON strings on disk — same limitation as Python unless columns are typed ARRAY / scalars.
...
**Next:** [Reading materialized training data](#sec-read-strategies) for OBJECT/ARRAY/VECTOR handling, then [Choose a pipeline](#sec-e2e-pipelines) (A–E) for full workflows.
...
Pipeline steps **link to those sections** for extract/decode details rather than repeating them. Steps assume a **multilang Workspace notebook**: snowflakeR for Feature Store APIs, RSnowflake for reads, optional **ADBC** in R ([ADBC setup](#sec-adbc-semistructured)).
