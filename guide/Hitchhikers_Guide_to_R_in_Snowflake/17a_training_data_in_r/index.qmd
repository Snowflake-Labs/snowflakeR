...
It complements [Feature Store](../17_feature_store/index.qmd) (catalog and training APIs), [Authoring Feature Views](../17b_authoring_feature_views/index.qmd) (warehouse SQL/dbplyr for `feature_df`), and [RSnowflake: Query](../13_rsnowflake_query/index.qmd) (dbplyr push-down, `compute()`, bulk I/O).

### Scalars first, semi-structured when you need them {#sec-scalar-vs-semi}

**Use scalar columns:** Most Feature Views and training data sets use ordinary scalar columns, one Snowflake column per feature, point-in-time join via `sfr_generate_training_data()`, read with RSnowflake + ADBC as native R types ([Pipeline A](#sec-pipeline-a)). That is the default and works well for **up to a few hundred columns** after multi–Feature View joins.
