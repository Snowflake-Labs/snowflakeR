...
This chapter explains how the modes differ, what Snowflake objects each mode needs, and how work flows from your R session to workers and back. For a full many-model forecasting walkthrough, see [Many-Model Patterns](../21_many_model/index.qmd).
...
Reference Dockerfiles ship with snowflakeR under [`inst/workers/`](https://github.com/Snowflake-Labs/snowflakeR/tree/main/inst/workers) (see `?sfr_dosnowflake_build_image`).
...
In **many-model feeder** scenarios (screen-then-refine), additional rows can be **inserted dynamically** while workers run — e.g. only the worst-performing SKUs from a first pass — without rebuilding the entire Task graph. See [Many-Model Patterns](../21_many_model/index.qmd).
...
Package worker templates (`inst/workers/worker_queue.R`) implement a **dual-path** pattern: prefer volume mount, fall back to `GET`/`PUT` only when a JDBC-capable driver is present (not RSnowflake in SPCS).
...
For teams already using the [**crew**](https://github.com/A2Ai/crew) orchestration ecosystem, snowflakeR exposes **`crew_controller_spcs()`** — a long-lived controller that launches SPCS workers speaking **mirai** over the network.
...
Runnable monitors: `workspace_parallel_spcs_demo.ipynb` and `streamlit_parallel_demo_monitor.py` in [snowflakeR/inst/notebooks](https://github.com/Snowflake-Labs/snowflakeR/tree/main/inst/notebooks). See [Appendix C: Troubleshooting](../appendices/C_troubleshooting/index.qmd).
...
| `snowflaker_parallel_spcs_config.yaml` | Environment config template |
