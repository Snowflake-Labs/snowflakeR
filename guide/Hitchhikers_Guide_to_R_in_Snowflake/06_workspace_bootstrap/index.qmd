...
After [Workspaces & Notebooks](../05_workspaces_overview/index.qmd), enable R in notebooks with **snowflake-notebook-multilang**. Most enterprises follow **two phases**:

**1. Organisation onboarding (usually IT or platform engineering)** — Your Snowflake team builds a **[Custom Runtime Environment (CRE)](../09_custom_runtime_and_ml_jobs/index.qmd#sec-cre-e2e)**: a versioned container image with R, `%%R`, snowflakeR, RSnowflake, and your approved package set pre-installed. They register it in Snowflake (`cre@<your_org_name>`) and grant notebook roles access. Analysts then open Workspace with **no bootstrap cell** (v2 images) and go straight to `%%R` — startup is fast once the notebook container is running. This is the path most production teams adopt after a short pilot.
...
You may use **both**: bootstrap during evaluation; CRE once your organisation’s image is live.
...
| **Typical timing** | After pilot; ongoing image updates when packages change | Day one; before org CRE exists |
| **First `%%R`** | Seconds (after container is up) — v2 auto-registers on kernel start | ~60s typical bootstrap; longer with extra packages |
...
Use the bootstrap script **`sfnb_setup.py`** from [snowflake-notebook-multilang](https://github.com/Snowflake-Labs/snowflake-notebook-multilang) (same file is also in [snowflakeR/inst/notebooks/](https://github.com/Snowflake-Labs/snowflakeR/tree/main/inst/notebooks)).
...
| Git repo cloned into Workspace (typical) | File is under `snowflakeR/inst/notebooks/` — see [§filesystem paths](#sec-filesystem) below |
...
## TODO: Screenshot — Bootstrap cell

Capture: Python cell running `setup_notebook()` with successful output.
:::
...
::: {.callout-warning}
Never compile `snowflakeR` / `RSnowflake` from source inside Workspace. Build tarballs with `R CMD build --no-build-vignettes` locally.
:::
...
### Bootstrap pattern for notebooks outside `snowflakeR/inst/notebooks/`
...
If you are **enabling R for an organisation** (not only your own notebook), plan a one-time **CRE onboarding** project: build an image with [snowflake-notebook-multilang](https://github.com/Snowflake-Labs/snowflake-notebook-multilang) (`docker/create_cre.sh` + `cre_profile.yaml`), push to your Snowflake image repository, register `cre@<org_name>`, and grant `USAGE` to notebook roles. Reference **v2** registers `%%R` on kernel start (empty git repo is fine), includes **snowflakeR**, **RSnowflake**, and optional **ADBC**.
...
::: {.callout-warning}
Git-connected projects with **renv** (`.Rprofile` → `renv/activate.R` at the repo root) can hijack R on the `/filesystem` mount. Use a CRE **v2** image or `enable_r_cells()` from the bootstrap helpers — never raw `%load_ext rpy2.ipython` on those repos.
:::

## Companion notebook

- [`workspace_quickstart.ipynb`](https://github.com/Snowflake-Labs/snowflakeR/blob/main/inst/notebooks/workspace_quickstart.ipynb)
