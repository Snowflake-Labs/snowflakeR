...
This appendix documents what a live **Workspace Notebook container** looks like inside (validated on snowbooks + sfnb CRE v2, May 2026). Names and paths may shift slightly by Snowflake release; the **shape** (nbctl → jupyter-server → uvenv ipykernel → FUSE `/filesystem`) is the stable mental model.
...
| Check | Healthy CRE v2 |
|-------|----------------|
| `/opt/sfnb/CRE_VERSION` | e.g. `v2` |
...
| **CRE bootstrap** | IPython `startup/` hook (what v2 does) | kernelspec install + UI integration |
...
CRE v2 already exploits the same hooks Jupyter extensions use:
...
CPU sfnb CRE v2 does **not** need changes for GPU; ship a **separate GPU-tagged image** and CRE object when you are ready to test.
