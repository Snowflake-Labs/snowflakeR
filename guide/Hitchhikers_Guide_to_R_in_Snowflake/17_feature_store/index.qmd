...
From R, build bundles on the warehouse with RSnowflake-only dplyr helpers such as `object_agg()` and `array_agg()` — see [RSnowflake Query §RSnowflake-only translations](../13_rsnowflake_query/index.qmd#sec-snowflake-dbplyr), [§VECTOR exports](../13_rsnowflake_query/index.qmd#sec-vector), [§Wide training sets](../13_rsnowflake_query/index.qmd#sec-wide-training), and [§Reading training data in R](../13_rsnowflake_query/index.qmd#sec-feature-store-read). Register the resulting SQL as `feature_df`; use `sfr_generate_training_data()` with `save_as` for large or wide joins rather than pulling the full matrix into R on every run.
