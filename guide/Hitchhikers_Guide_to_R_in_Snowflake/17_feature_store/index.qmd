...
From R, build bundles on the warehouse with RSnowflake-only dplyr helpers such as `object_agg()` and `array_agg()` — see [RSnowflake Query — dbplyr](../13_rsnowflake_query/index.qmd#sec-snowflake-dbplyr) and [Authoring Feature Views](../17b_authoring_feature_views/index.qmd#sec-wide-bundles).
