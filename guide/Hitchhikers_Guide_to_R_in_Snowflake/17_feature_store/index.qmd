...
See also [MLOps on Snowflake](../15_mlops_on_snowflake/index.qmd) for where Feature Store sits in the lifecycle, [Feature Engineering with R](../17a_feature_engineering_with_r/index.qmd) for training export pipelines (OBJECT/ARRAY/VECTOR → R), and [Model Registry](../18_model_registry/index.qmd) for logging models trained on Feature Store datasets.
...
Training uses a **spine** (entity keys + event timestamps + labels). Inference typically uses current or latest feature values — no labels, no point-in-time join across history.
...
flowchart LR
  ...
  subgraph paths [Training then inference]
    direction TB
    subgraph train [Training path]
      direction TB
      SPINE[Spine labels plus event_ts]
      TD[Training data or Dataset]
      SPINE --> TD
    end
...
When a feature view would need **hundreds of scalar columns**, prefer **OBJECT** or **ARRAY** bundles (and, for training export at scale, **ARRAY → `::VECTOR`** columns) instead of a wide table. Snowflake documents the pattern in [Implementation Guide §12.10 — Wide & sparse](https://snowflake-labs.github.io/snowflake-featurestore-imp-guide/12_advanced_patterns/#sec-wide-sparse).

From R, build bundles on the warehouse with RSnowflake-only dplyr helpers such as `object_agg()` and `array_agg()` — see [RSnowflake Query — dbplyr](../13_rsnowflake_query/index.qmd#sec-snowflake-dbplyr).

For **end-to-end training export** (scalar-wide ASOF → three ARRAY/VECTOR packs, ARRAY-native views, OBJECT flattening, Python decode), use the pipeline table in [Feature Engineering with R — End-to-end pipelines](../17a_feature_engineering_with_r/index.qmd#sec-e2e-pipelines) (Pipelines A–E).
...
For **batch scoring** or joins against **offline** Feature Views, fetch **latest** values (no labels, no `spine_timestamp_col`) via `sfr_retrieve_features()`:
...
For **sub-minute, key-based** serving, use the **online** Hybrid Table instead — [Online serving](#sec-online) (`sfr_read_feature_view(..., store_type = "ONLINE")` or SQL on the OFT).
...
### Retrieve values — snowflakeR → snowflake-ml-python
...
| `sfr_retrieve_features()` | Offline Feature Views | Join a **spine** to features (batch scoring / latest offline values) — not the OFT API |
...
### Retrieve values — SQL on the Online Feature Table

The OFT is a queryable table in your Feature Store schema (naming is managed by the platform — often a `*_OFT` suffix). Discover the exact name in **Snowsight** (Feature Store UI) or metadata commands, then query like any Hybrid Table:
...
From R with **RSnowflake** (no Feature Store API — useful for SQL-only services or joins in the warehouse):
...
Use **bind parameters** (`?`) for high-QPS serving so Snowflake can reuse plans ([Implementation Guide — parameter binding](https://snowflake-labs.github.io/snowflake-featurestore-imp-guide/08_online_features/index.html#sec-8-9-4-parameter-binding)).
...
- **Real-time scoring:** `sfr_read_feature_view(..., ONLINE)` or **SQL** against the OFT → **latest** values per key, bounded by `target_lag`.
:::
