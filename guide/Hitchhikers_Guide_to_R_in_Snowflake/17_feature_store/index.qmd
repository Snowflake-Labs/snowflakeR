...
From R, build bundles on the warehouse with RSnowflake-only dplyr helpers such as `object_agg()` and `array_agg()` — see [RSnowflake Query §RSnowflake-only translations](../13_rsnowflake_query/index.qmd#sec-snowflake-dbplyr) and [§VECTOR](../13_rsnowflake_query/index.qmd#sec-vector). Register the resulting SQL as `feature_df`; use `sfr_generate_training_data()` for point-in-time joins rather than `collect()` on the full training set.
