...
::: {.callout-important}
If you are moving a **large volume of data** into Snowflake from R, prefer a **driver-based** workflow (**JDBC/ODBC `PUT` + `COPY`**, or RSnowflake **ADBC** bulk upload when installed) over SQL API bind inserts. RSnowflake's `upload_method = "auto"` already prefers ADBC when available — see [Query — Bulk write routing](../13_rsnowflake_query/index.qmd#sec-bulk). For terabyte-scale file loads outside DBI, classic drivers remain the standard pattern.
:::
