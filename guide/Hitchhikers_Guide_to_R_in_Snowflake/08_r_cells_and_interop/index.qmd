...
- Bootstrap runs once in **Python** (`setup_notebook()`)
...
sequenceDiagram
  ...
  U->>PY: Run bootstrap cell
  PY->>BOOT: micromamba, EAI, %%R magic
...
| Event | What happens |
|-------|----------------|
| **Start session** | Empty Python kernel; R not installed until bootstrap |
| **After bootstrap** | R available; `%%R` magic registered |
...
Workspace's HTML renderer can insert **extra line breaks** between lines of R `print()` output — tables look stretched and hard to read. snowflakeR provides helpers that emit output in a single clean pass:

| Function | Replaces | Use for |
|----------|----------|---------|
| `rprint(x)` | `print(x)` | Models, lists, data frames |
| `rview(df, n = 10)` | `head(df)` | Quick row preview |
| `rglimpse(df)` | `glimpse()` / `str()` | Column types and sample values |
| `rcat(...)` | `cat()` | Messages without double spacing |
...
Bootstrap sets **`options(width = 200)`** and **`options(tibble.width = Inf)`** so wide tibbles do not wrap aggressively. Adjust from Python if needed:
...
|------|----------|---------|---------|
| Bootstrap | Python | Container kernel | `setup_notebook()` |
...
## Installing extra R packages in-session {#sec-packages}

Bootstrap installs packages from YAML. For ad hoc additions in a session:
...
Workspace has **no interactive stdin** — always set `repos`, use `quiet = TRUE`, and suppress startup messages. Packages with **system libraries** (e.g. `sf` → GDAL) may fail with plain `install.packages()`; add **conda-forge** equivalents (`r-sf`) to YAML instead.

Interactive installs **do not survive** container recycle — add recurring packages to `snowflaker_*.yaml` or bake into a [CRE](../09_custom_runtime_and_ml_jobs/index.qmd).

::: {.callout-warning}
Pin **`r-reticulate>=1.25`** in config. Older reticulate versions can **segfault** when loaded with rpy2 in the same process.
:::
