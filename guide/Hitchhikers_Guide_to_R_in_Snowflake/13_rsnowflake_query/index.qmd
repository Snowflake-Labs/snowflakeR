...
::: {.callout-tip}
**Feature Store training exports** (OBJECT/ARRAY/VECTOR pipelines, ADBC decode, wide ASOF joins) live in [Feature Engineering with R](../17a_feature_engineering_with_r/index.qmd) — not here. This chapter focuses on everyday dbplyr analytics.
:::
...
Inspect row counts without pulling data:
...
Stratified sample for exploratory plots:
...
RSnowflake can **build** OBJECT/ARRAY bundles with `object_agg()` and `array_agg()` in dplyr ([§RSnowflake-only translations](#sec-snowflake-dbplyr)). How those columns **round-trip into R** (JSON vs Arrow list columns, VECTOR packs, Feature Store `save_as`) is covered in [Feature Engineering with R](../17a_feature_engineering_with_r/index.qmd).
