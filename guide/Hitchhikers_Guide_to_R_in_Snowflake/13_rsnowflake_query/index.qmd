...
- Understand how VARIANT / OBJECT / ARRAY / VECTOR columns appear in R after `collect()`
...
## VARIANT, OBJECT, ARRAY, and VECTOR in R {#sec-semi-structured-read}

### What `collect()` returns today

RSnowflake maps Snowflake types in `type-map.R` when results come through the **SQL API** (JSON):

| Snowflake type | R type after `collect()` / `dbGetQuery()` | Notes |
|----------------|-------------------------------------------|-------|
| VARIANT, OBJECT, ARRAY | **character** | JSON text per cell |
| VECTOR | **character** | JSON (or string) representation — **not** a native R matrix column |
| TEXT, GEOGRAPHY, GEOMETRY | character | |
| FIXED, REAL, BOOLEAN, timestamps | integer / double / logical / POSIXct | As usual |

So `collect()` on a wide training export gives you **columns of JSON strings**, not an `n × p` numeric matrix. Parse in R when needed:

```r
library(jsonlite)

row <- training[1, ]
feat_obj <- fromJSON(row$FEATURE_OBJECT)
# named numeric vector — then cbind/bind_cols for modeling
```

For very large exports, prefer **keeping transforms in SQL** (narrow/aggregated result) or **ADBC** bulk reads (`options(RSnowflake.backend = "adbc")`) and measure whether Arrow delivers richer types for your account — the SQL API path is the conservative baseline documented here.

### Why this matters for Feature Store + wide features

| Approach | In Snowflake | At R `collect()` |
|----------|--------------|------------------|
| Wide table (500+ scalar columns) | Simple SQL | Many numeric columns; heavy compile + transfer |
| OBJECT bundle per entity | Few columns; [recommended in FS guide](https://snowflake-labs.github.io/snowflake-featurestore-imp-guide/12_advanced_patterns/#sec-wide-sparse) | One JSON string per row — parse client-side |
| ARRAY → `::VECTOR(FLOAT, n)` at query time | O(1) compile vs per-column extract | Still character in R via SQL API today; Python+Arrow can get `fixed_size_list` |

RSnowflake's advantage for R users is **pushing** `object_agg`, `array_agg`, and approximate aggregates into SQL via dplyr — not requiring Python for the transform step.

---

## VECTOR type {#sec-vector}

### Do we support VECTOR?

**Yes as a Snowflake column type you can query.** RSnowflake does not block `VECTOR(FLOAT, n)` / `VECTOR(INT, n)` in SQL.

**In R today:** VECTOR columns map to **`character`** (same family as VARIANT/OBJECT/ARRAY on the SQL API path). There is **no** built-in conversion to an R `matrix` or packed numeric vector on `collect()`.

| Layer | VECTOR support |
|-------|----------------|
| SQL / `dbGetQuery()` | Full — use casts, `VECTOR_AVG`, `VECTOR_SUM`, etc. in SQL text |
| dbplyr helpers | **No** dedicated `vector_avg()` wrappers yet — use `sql()` or raw SQL |
| `collect()` → modeling matrix | **Manual** — parse JSON or build SQL that returns scalars/ARRAY instead |

Snowflake stores VECTOR as a fixed-width numeric sequence (up to **4,096** dimensions). That is the format SnowCAT wide-table work targets for efficient export to Python (Arrow `fixed_size_list` → NumPy). See internal research in `sc-sf-adaptive-vw-benchmark` / `WIDE_DATA_BRIEFING.md` (engineering repo).

### Three-vector export pattern (ARRAY → VECTOR)

Wide-table benchmarks found it more efficient to export **three ARRAY columns**, cast to **VECTOR** at query time, instead of thousands of scalar columns or per-field OBJECT extraction:

1. **`features_float`** — `VECTOR(FLOAT, n)` for numeric / imputed floats  
2. **`features_int`** — `VECTOR(INT, n)` for categoricals encoded as ordinals  
3. **`null_mask`** — `VECTOR(INT, ceil(n/32))` bitmask for which features were NULL  

Compilation stays **O(1)** in feature count because the planner sees three expressions, not one per feature. Example (run on warehouse; adjust names):

```r
training_vectors <- dbGetQuery(con, "
  SELECT
    entity_id,
  ...
  FROM FEATURE_STORE.ML.ARRVEC_TRAINING_EXPORT
  WHERE event_date = '2026-01-01'
  LIMIT 10000
")

# SQL API path: columns are character JSON — decode per row or in a follow-up R step
str(training_vectors)
```

Building `features_float_arr` / `features_int_arr` is usually done in **SQL or Dynamic Tables** (see Feature Store guide §12.10 and ARRVEC patterns), not row-by-row in R. R's role is **orchestration**: register views, `generate_training_data()`, and selective `collect()` for validation.

### Practical guidance for R + wide ML

| Goal | Prefer |
|------|--------|
| Feature View with 30–50 columns | Scalar columns + dbplyr; `collect()` is fine |
| 100+ features, shared definitions | OBJECT or ARRAY bundles in Feature View SQL; avoid `SELECT *` with 500 literals |
| Training export for Python/pytorch | Often **Python cell** or batch job with ADBC/Arrow; R validates SQL and row counts |
| Proof-of-concept in R only | `collect()` small `LIMIT`, `jsonlite::fromJSON` on OBJECT column, then `as.matrix()` |

::: {.callout-tip}
## Related chapters

- [Feature Store](../17_feature_store/index.qmd) — `sfr_generate_training_data()`, entities, feature views  
- [Feature Store Implementation Guide — Wide & sparse](https://snowflake-labs.github.io/snowflake-featurestore-imp-guide/12_advanced_patterns/#sec-wide-sparse) — platform patterns (Python examples; SQL is the same)  
