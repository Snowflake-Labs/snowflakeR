...
That is why bootstrap often fails with cryptic timeouts: **`setup_notebook()` must download software from the internet** (micromamba, R, CRAN packages, GitHub tarballs, pip packages for rpy2). Snowflake governs that egress through **network rules** and **External Access Integrations (EAI)**.

This chapter explains what those objects are, how `setup_notebook()` discovers or creates them, what you must do in Snowsight after an admin runs SQL, and how enterprise **artifact mirrors** simplify allowlists.

::: {.callout-note}
**Folder vs book numbering:** This file lives at `07_network_and_eai/` in the repo. In the rendered book it typically appears as **Chapter 8** (after Introduction, Platform, Architecture, Local setup, IDE, Workspaces, and Bootstrap). Cross-references in this guide use **chapter titles**, not folder numbers.
:::
...
Even after SQL succeeds, **enable external access on the session**:

1. Open the notebook
2. **Connected** (or session menu) → **Edit**
3. **External Access** → toggle your EAI **ON** → **Save**
4. Re-run bootstrap if the first attempt failed mid-download

::: {.callout-note appearance="minimal"}
## TODO: Screenshot — EAI toggle

Capture: Connected/Edit dialog with External Access integration enabled.
:::

**One-time per account setup, reuse across notebooks:** The same EAI can serve many notebooks once created — each new session still needs the **toggle** unless your org automates attachment.
...
## Enterprise mirrors {#sec-mirrors}

If all package traffic routes through **Artifactory**, **Nexus**, or Snowflake-internal mirrors, the allowlist collapses to **one hostname** instead of dozens of public CDNs.
...
See [custom_mirrors.md](https://github.com/Snowflake-Labs/snowflake-notebook-multilang/blob/main/docs/custom_mirrors.md).
...
|----------|----------------|
| **Cold bootstrap** | Required for CRAN/GitHub/conda/PyPI |
