...
Examples use **SQL** and **dbplyr** tabs (`group="fe-sql-dplyr"`) — the same pattern as [RSnowflake: Query](../13_rsnowflake_query/index.qmd#sec-dbplyr) and [Training Data in R](../17a_training_data_in_r/index.qmd#sec-sql-dplyr-tabs). Use **dbplyr** when you already work in R with `tbl()`; use **SQL** when you share DDL with other teams or generate views from a manifest.
...
- RSnowflake + dbplyr ([RSnowflake: Query](../13_rsnowflake_query/index.qmd))
- Optional: [`workspace_feature_store.ipynb`](https://github.com/Snowflake-Labs/snowflakeR/blob/main/inst/notebooks/workspace_feature_store.ipynb)
...
| **SQL `OVER (...)` / dbplyr windows** | Custom frame bounds, `ROW_NUMBER`, joins, non-standard logic — [Multi-step pipelines](#sec-multi-step), [RSnowflake Query — windows](../13_rsnowflake_query/index.qmd#sec-windows) |
...
For production, prefer **permanent** tables or views (`temporary = FALSE`, fully qualified names) when the intermediate must survive beyond the session. See [RSnowflake Query — Multi-step ETL](../13_rsnowflake_query/index.qmd#sec-etl-temp).
...
See [RSnowflake Query — OBJECT bundles](../13_rsnowflake_query/index.qmd#sec-snowflake-dbplyr) for `object_agg()` / `array_agg()` details.
...
- Vignette [`feature-store`](https://snowflake-labs.github.io/snowflakeR/articles/feature-store.html) — full API reference including advanced registration options
