...
## RSnowflake vs `odbc` + Snowflake ODBC {#sec-odbc}
...
|-------|----------------|----------------------------|
| R package | [RSnowflake](https://github.com/Snowflake-Labs/RSnowflake) | CRAN [`odbc`](https://odbc.r-dbi.org/) |
| Under the hood | [SQL REST API](../11_rsnowflake_sql_api/index.qmd) (+ optional **ADBC** for bulk I/O) | **unixODBC** / iODBC → [Snowflake ODBC driver](https://docs.snowflake.com/en/developer-guide/odbc/odbc) |
...
| **Client `GET` / `PUT` SQL** | Not on SQL API — use ADBC bulk, Snowpark in Workspace, or stage files another way ([Query §limits](../13_rsnowflake_query/index.qmd#sec-limits)) | ODBC/JDBC stack often used for stage-oriented workflows teams already know |
| **Bulk load from R** | `"auto"` → ADBC / Snowpark / literal INSERT ([Query §bulk](../13_rsnowflake_query/index.qmd#sec-bulk)) | Driver-dependent; no RSnowflake-specific routing |
...
| You need **SQL API** semantics documented in [chapter 11](../11_rsnowflake_sql_api/index.qmd) | You need **driver-level** stage/file workflows from R that assume ODBC |
...
See [RSnowflake in Workspace §bootstrap](../14_rsnowflake_workspace/index.qmd#sec-order).
