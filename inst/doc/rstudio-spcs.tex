...
\textbf{snowflakeR} and \textbf{RSnowflake} pre-installed. This vignette
is the \textbf{technical reference} for the deploy kit shipped in this
package:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{kit }\OtherTok{\textless{}{-}} \FunctionTok{system.file}\NormalTok{(}\StringTok{"rstudio{-}spcs"}\NormalTok{, }\AttributeTok{package =} \StringTok{"snowflakeR"}\NormalTok{)}
...
Browser ──HTTPS ingress──► SPCS service (RStudio :8787)
                              ├── Native R session
                              ├── RSnowflake (SQL API / DBI)
                              ├── snowflakeR + reticulate
...
Provision stages / pool & \texttt{provision.sql} \\
Build EAI (Image Builder) &
\texttt{provision\_image\_builder\_eai.sql} \\
...
All paths are under \texttt{inst/rstudio-spcs/} in the snowflakeR
repository.

\subsection{RSnowflake in RStudio}\label{rsnowflake-in-rstudio}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(DBI)}
\FunctionTok{library}\NormalTok{(RSnowflake)}
...
spec. See
\textbf{\texttt{vignette("spcs-custom-services",\ package\ =\ "RSnowflake")}}
for REST API limits (\texttt{USE\ WAREHOUSE},
\texttt{GET}/\texttt{PUT}).

\subsection{\texorpdfstring{snowflakeR:
\texttt{sfr\_connect\_spcs()}}{snowflakeR: sfr\_connect\_spcs()}}\label{snowflaker-sfr_connect_spcs}
...
\item
  Calls \texttt{snowflakeR}'s Python bridge with
...
\texttt{prepare\_build\_ctx.sh} builds a \textbf{snowflakeR} tarball
locally and copies it into the flat build context. \textbf{RSnowflake}
...
\item
  Kit README:
  \texttt{system.file("rstudio-spcs/README.md",\ package\ =\ "snowflakeR")}
\end{itemize}
