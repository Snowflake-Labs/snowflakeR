...
**Driver scale (optional keys under `parallel_lab`):** `demo_forecast_n_skus`, `demo_tasks_chunks_per_job`, `demo_queue_n_workers`, `demo_queue_chunks_per_job` — defaults in `snowflaker_parallel_spcs_config.yaml` (for example 2000 SKUs, modest parallelism for a first run).

The checked-in `snowflaker_parallel_spcs_config.yaml` uses a generic lab-style naming pattern with `create_synthetic_series_table: true` so the demo can run without your own source tables.
