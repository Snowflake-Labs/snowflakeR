...
try:
    ...
    import sys; sys.path.insert(0, "../../../snowflake-notebook-multilang/src")
    from sfnb_multilang import install

install(languages=["r"])
```

This installs R, `rpy2`, and base R packages via `micromamba` + conda-forge.
Everything is installed in **user-space** -- no `sudo` or root access is
required or used. Nothing modifies the container base image or system Python
environment.

For more advanced configurations (ADBC, DuckDB, custom packages, Scala/Julia),
pass a YAML config file instead:
...
| Issue | Solution |
|-------|----------|
| `rpy2` not found | Run `install(languages=["r"])` from `sfnb_multilang` |
