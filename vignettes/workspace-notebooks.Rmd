...
```yaml
# snowflaker_config.yaml -- all sections optional
context:
...
When your Git project includes a **local snowflakeR checkout**, `sfnb_setup.py`
may set `SNOWFLAKER_PATH` to that source tree for fast iteration. Otherwise
the `else` branch installs from GitHub releases via `pak`.
...
Supporting files in the same directory include per-notebook `snowflaker_*.yaml`
configs (for example `snowflaker_parallel_spcs_config.yaml` for the parallel
...
these through **`snowflaker_parallel_spcs_config.yaml`** and shared Python
