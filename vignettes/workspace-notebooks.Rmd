...
This installs R, `rpy2`, base R packages, and optionally the ADBC Snowflake
driver (including Go compiler) via `micromamba`. Use `--basic` instead of
`--adbc` to skip ADBC installation (faster, but no Arrow-native bulk I/O).
...
**External Access Integration (EAI):** The setup script downloads packages from
...
that allows these hosts. See the full host table and example SQL in
`internal/prd_eng/workspace_notebooks_eai_requirements.md`.
