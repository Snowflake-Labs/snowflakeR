...
Python Kernel (Jupyter)
  ...
  |     |-- snowflakeR (R package, ML platform features)
  |     |     |-- reticulate (R-Python bridge)
  |     |           |-- snowflake-ml-python (Snowflake ML SDK)
  |     |                 |-- Snowpark Session (built-in)
  |     |
  |     |-- RSnowflake (R package, DBI/dbplyr, optional)
  |           |-- Snowflake SQL API v2 (reads, small writes)
  |           |-- reticulate -> write_pandas (bulk writes via SPCS path)
  |           |-- ADBC (optional, Arrow-native reads)
  |
  |-- Active Snowpark Session (auto-detected by sfr_connect() / RSnowflake)
```

`snowflakeR` uses `reticulate` to call the Snowflake ML Python SDK,
while `RSnowflake` connects directly to Snowflake via HTTPS (no Python
needed). Both work in Workspace Notebooks. For ML workflows (Model
Registry, Feature Store), use `snowflakeR`. For standard DBI/dbplyr
database operations, install `RSnowflake` and use `sfr_dbi_connection()`.
...
### Step 1: Install the R environment and register `%%R` magic
...
bootstrap file that auto-detects the environment and installs the toolkit if
needed:

```python
# Python cell -- run once per session (~1-2 minutes)
from sfnb_setup import install_r
install_r()
```

This single call installs R via `micromamba` + conda-forge, registers the `%%R`
cell magic via `rpy2`, and configures the R environment. Everything is installed
in **user-space** -- no `sudo` or root access is required.

For more advanced configurations (ADBC, DuckDB, custom packages, Scala/Julia),
pass keyword arguments or a YAML config file:

```python
install_r(r_adbc=True)                      # enable ADBC
install_r(config="configs/snowflaker.yaml")  # use preset config
```

See the preset configs in the `snowflake-notebook-multilang` repo under
`configs/` for examples.

**Alternative (no sfnb_setup.py):** If you prefer not to use the bootstrap file,
you can inline the setup directly in a notebook cell (~25 lines):
...
try:
    ...
    subprocess.check_call([sys.executable, "-m", "pip", "install", "-q", "sfnb-multilang"])
    from sfnb_multilang import install
...
`SNOWFLAKER_PATH` to the local package source. From the public repository,
...
(see `configs/snowflaker.yaml` in the `sfnb-multilang` toolkit) for
...
When writing large data from R in Workspace Notebooks, RSnowflake
...
Outside Workspace, RSnowflake uses ADBC (if available) for bulk writes
via the public endpoint. See the RSnowflake `getting-started` vignette
for the full `upload_method` options table.

## PAT management

Most `snowflakeR` operations use the Snowpark session directly (no extra
...
```python
# Cell 1 (Python): Install R and register %%R magic
from sfnb_setup import install_r
install_r()
```
...
| Issue | Solution |
|-------|----------|
| `rpy2` not found | Run `from sfnb_setup import install_r; install_r()` |
