...
| Python/R env | You manage | `sfnb_multilang.install()` |
...
1. **Install R** (run once per session, ~1-2 minutes):

```python
# Python cell
# Public standalone: !pip install sfnb-multilang
from sfnb_multilang import install
install(languages=["r"])
```

   For advanced configurations (ADBC, custom packages, Scala/Julia), use a
   YAML config file: `install(config="configs/snowflaker.yaml")`.

2. **Register `%%R` magic and load `snowflakeR`:**
